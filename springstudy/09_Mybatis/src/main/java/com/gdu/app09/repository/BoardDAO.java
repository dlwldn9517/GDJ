...
public class BoardDAO {
	...
	public List<BoardDTO> selectAllBoards() {
		...
		List<BoardDTO> boards = jdbcTemplate.query(sql, new BeanPropertyRowMapper<>(BoardDTO.class));	// DB의 Row와 BoardDTO.class의 필드가 같으면 알아서 List에 추가
		return boards;
	}
	
	public BoardDTO selectBoardByNo(final int board_no) {	// 예전에 매개변수 해킹 시도가 있어서 final이 붙을 수 있다.
		String sql = "SELECT BOARD_NO, TITLE, CONTENT, WRITER, CREATE_DATE, MODIFY_DATE FROM BOARD WHERE BOARD_NO = ?";
		BoardDTO board = jdbcTemplate.queryForObject(sql, new BeanPropertyRowMapper<>(BoardDTO.class), board_no);	// queryForObject() 쿼리결과 1개
		return board;
	}
