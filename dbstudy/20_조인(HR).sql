...
SELECT E.EMPLOYEE_ID, E.LAST_NAME, D.DEPARTMENT_NAME
  FROM DEPARTMENTS D INNER JOIN EMPLOYEES E
    ON D.DEPARTMENT_NAME = 'Executive';
...
SELECT E.EMPLOYEE_ID, E.LAST_NAME, J.JOB_ID
  FROM EMPLOYEES E INNER JOIN JOB_HISTORY J
    ON J.JOB_HISTORY = E.JOB_HISTORY;


-- 4. 부서별로 사원수와 평균연봉을 DEPARTMENT_NAME과 함께 조회하시오.
--    평균연봉은 정수로 절사하고, 사원수의 오름차순 정렬하시오.
SELECT COUNT(*) AS 사원수, FLOOR(AVG(E.SALARY)) AS 평균연봉, D.DEPARTMENT_NAME
  FROM EMPLOYEES E INNER JOIN DEPARTMENTS D
 GROUP BY FLOOR(AVG(SALARY)), DEPARTMENT_NAME
 ORDER BY COUNT(*);
