/*
    ...
    1. Trigger
    2. DML (INSERT, UPDATE, DELETE) 수행 후 트리거가 자동으로 수행된다.
    3. DML 직전에 수행되는 BEFORE 트리거, 직후에 수행되는 AFTER 트리거가 있다.
    4. 기본적으로 작업 수행 행(Row) 단위로 트리거가 적용된다.
    5. 형식
...
CREATE OR REPLACE TRIGGER TRIG1
    ...
    ON DEPARTMENT   -- DEPARTMENT 테이블을 UPDATE할 때 동작한다.
    FOR EACH ROW    -- UPDATE되는 행마다 트리거가 동작한다.
BEGIN
...
CREATE OR REPLACE TRIGGER TRIG2
    AFTER
    INSERT OR UPDATE OR DELETE  -- 작성 순서 없다.
    ON DEPARTMENT
    FOR EACH ROW
BEGIN
    IF INSERTING THEN
...
CREATE TRIGGER OR REPLACE RETIRE_TRIG
