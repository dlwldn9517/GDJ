...
/*
    ...
    3. 서브쿼리는 메인쿼리에 괄호()를 이용해서 포함시킨다.
    4. 항상 서브쿼리를 먼저 실행하고, 서브쿼리 실행 결과를 메인쿼리에서 사용한다.
    5. 사용되는 절에 따른 구분
     1) SELECT절 : 스칼라 서브쿼리
     2) FROM절   : 인라인뷰
     3) WHERE절  : 서브쿼리
...
SELECT EMP_NO, NAME
  FROM EMPLOYEE
 WHERE ROWID = 'AAAFD4AABAAALDBAAC';
...
-- ROWNUM에 별명을 지정하고 해당 별명을 사용하면 된다.
-- 3 : SELECT
...
-- 2) 정렬과 ROWNUM 이용
--  (1) 연봉의 내림차순 정렬을 수행(가장 높은 연봉이 1번째 행이 된다.)
...
SELECT A.EMP_NO, A.NAME, A.HIRE_DATE
  FROM (SELECT RANK() OVER(ORDER BY HIRE_DATE) AS 입사일순, EMP_NO, NAME, HIRE_DATE
          FROM EMPLOYEE) A
 WHERE A.입사일순 BETWEEN 3 AND 4;



/* 기타 서븤쿼리 : CREATE, UPDATE, DELETE 등에서 활용 */
...
UPDATE EMPLOYEE2
   SET NAME = (SELECT NAME FROM EMPLOYEE WHERE EMP_NO = 1004)
     , GENDER = (SELECT NAME FROM EMPLOYEE WHERE EMP_NO = 1004)
WHERE EMP_NO = 1001;
