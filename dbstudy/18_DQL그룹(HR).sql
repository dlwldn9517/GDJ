...
-- 2. SELECT절에서 조회할 칼럼은 "반드시" GROUP BY절에 존재해야 한다. 
...
-- 2. 그룹화 실패
-- EMPLOYEE_ID 조회하려면 GROUP BY절에 EMPLOYEE_ID가 있어야 한다.
SELECT EMPLOYEE_ID
  FROM EMPLOYEES
 GROUP BY DEPARTMENT_ID;
...
-- 4. 조건 지정
--  1) GROUP BY로 처리할 행(Row)은 적을수록 성능에 유리하다.
--  2) WHERE절 : GROUP BY 이전에 처리되므로 가능한 모든 조건은 WHERE절에서 처리한다.
--  3) HAVING절 : WHERE절로 처리할 수 없는 조건만 HAVING절에서 처리한다.

-- 1) 부서번호가 100 미만인 부서들의 연봉평균을 조회하기
--    (WHERE절과 HAVING절 모두 가능한 조건)
