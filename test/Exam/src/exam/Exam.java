...
public class Exam {
	...
	public static void q4() {
		...
		double average = (double)total / (double)end;
		
		if(end < 0) {
			System.out.println("평균 : 0");
		} else {
			System.out.println("0부터 " + end + "사이 모든 정수의 평균 : " + average);
		}
	...
	public static void q6() {
		...
		for(int n = 1; n <= 10; n++) {
			...
			System.out.println("출금 전 " + balance + "원, " + n + "회 출금 " + money + "원, 출금 후 " + (balance - money) + "원");
			balance -= money;
		}
	}
	...
	public static void q8() {
		...
		Scanner sc = new Scanner(System.in);
		System.out.println("변환 전 파일명 입력 >>> ");
		String file = sc.next();
			
		long timestamp = System.currentTimeMillis();
		
		int idxOfDot = beforeName.lastIndexOf(".");
		afterName = beforeName.substring(0,idxOfDot) + "_" + timestamp + beforeName.substring(idxOfDot);
	...
	public static void q10() {
		String pw = "1234abcd";  // 비밀번호
		
		
		Scanner sc = new Scanner(System.in);
		
		for(int n = 1; n <= 5; n++) {
		System.out.println("비밀번호 입력(" + n + "회) >>> ");
